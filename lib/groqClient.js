...
import { insertCommentaryData } from './singleStoreClient';
import { sendCommentary } from './kafkaClient';
...
export async function generateCommentaryWithGroq(encodedImage) {
  ...
  while (retries < MAX_RETRIES) {
    try {
      ...
      await insertCommentaryData(timestamp, commentary, embedding);

      // Send commentary to Kafka
      await sendCommentary({ timestamp, commentary, embedding });
